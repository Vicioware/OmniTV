...
- Exporta, de forma opcional, un único M3U con TODOS los vivos analizados
...
def load_source_urls(path: Path) -> list[str]:
    """Obtiene una URL por línea; ignora vacías y comentarios."""
    urls: list[str] = []
...
def main() -> int:
    ...
    ap.add_argument(
        "--skip-check",
        action="store_true",
        help="No verifica las fuentes; útil solo para depuración",
    )
    args = ap.parse_args()
    ...
    with tempfile.TemporaryDirectory(prefix="iptv-merge-") as tmp:
        ...
        for number, source_url in enumerate(source_urls, 1):
            ...
            if args.skip_check:
                ...
                if rc != 0:
                    source_errors += 1
                    print(f"  SKIP checker: código de salida {rc}", flush=True)
                    continue

                if not ok_path.exists() or not fail_path.exists():
                    source_errors += 1
                    print(
                        "  SKIP checker: no se generó playlist de vivos o muertos",
                        flush=True,
                    )
                    continue
    ...
    # Consolidar y quitar repetidos únicamente en los reportes.
    ...
    print(f"Nuevos añadidos a la maestra: {added}", flush=True)
    print(f"Total final de la maestra: {len(master_entries)}", flush=True)
    print(f"Maestra: {master_path}", flush=True)

    # Una fuente caída no detiene todo el proceso:
    # la maestra se actualiza con las demás fuentes válidas.
    return 0
