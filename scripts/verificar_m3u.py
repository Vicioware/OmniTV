...
def write_m3u(path: str, header: str, entries: List[Entry], reasons=None) -> None:
...
def main() -> int:
    ...
    if not shutil.which("ffmpeg"):
        sys.exit("ERROR: ffmpeg no está instalado o no está en el PATH.")
    ...
    try:
        ...
        sys.exit(f"ERROR: no se pudo leer la lista: {ex}")

    header, entries = parse_playlist(text)
    if not entries:
        sys.exit("No se encontraron entradas #EXTINF con URL.")
    ...
    print("\n=================== RESUMEN ===================")
    print(f"OK:        {len(oks)}  ->  {cfg.ok}")
    print(f"FAIL:      {len(fails)}  ->  {cfg.fail}")
    pend = total - len(oks) - len(fails)
    if pend:
        print(f"PENDIENTE: {pend} (solo en el CSV)")
    print(f"Informe:   {cfg.report}")
